...
def build_series(data, is_day):
    ...
    return {'dates':dates,'ohlc':ohlc,'vols':vols,'vc':vc,'mas':mas,
            'boll':[bup,bmid,blo],'raw':data}

DATA = {'15min': build_series(d15,False), '60min': build_series(d60,False),
        'daily': build_series(dd,True)}
...
#chart{{width:100%;height:65vh;border-radius:10px;background:#161b22;border:1px solid #30363d}}
...
<div class="header">
  <h1>全A指数</h1>
  <div class="tabs">
    <div class="tab active" data-per="15min">15分钟</div>
    <div class="tab" data-per="60min">60分钟</div>
    <div class="tab" data-per="daily">日线</div>
  </div>
...
function buildOpt(data) {{
  ...
  return {{
    ...
    tooltip:{{trigger:'axis',axisPointer:{{type:'cross'}},
      ...
      formatter:function(p){{const d=raw[p[0].dataIndex]; return '<b>'+d.t+'</b><br/>开: '+d.o.toFixed(2)+' 收: '+d.c.toFixed(2)+'<br/>高: '+d.h.toFixed(2)+' 低: '+d.l.toFixed(2)+'<br/>量: '+(d.v/1e8).toFixed(2)+'亿';}}}},
    grid:[{{left:'5%',right:'3%',top:'7%',height:'63%'}},{{left:'5%',right:'3%',top:'77%',height:'15%'}}],
    xAxis:[{{type:'category',data:dates,gridIndex:0,axisLine:{{lineStyle:{{color:'#30363d'}}}},axisLabel:{{color:'#8b949e',fontSize:10,interval:isDay?5:80}},splitLine:{{show:false}}}},
            {{type:'category',data:dates,gridIndex:1,axisLine:{{lineStyle:{{color:'#30363d'}}}},axisLabel:{{show:false}},splitLine:{{show:false}}}}],
    yAxis:[{{scale:true,gridIndex:0,splitLine:{{lineStyle:{{color:'#21262d',type:'dashed'}}}},axisLabel:{{color:'#8b949e',fontSize:10}}}},
...
print(f'15min: {len(d15)}  60min: {len(d60)}  daily: {len(dd)}')
